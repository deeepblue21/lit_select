...
def analyze_input_book(book_title):
    try:
        ...
        prompt = (
            f"Kontext:\n{blob}\n\nAnalysiere '{book_title}'. Gib NUR diese 5 Fakten, getrennt durch das Symbol | zurück:\n"
            f"Autor | Gattung | Genre-Anker | Tempo | Vibe\n"
            f"Beispiel: Benedict Wells | Roman | Coming-of-Age | Melancholisch | Nostalgisch"
        )
        res = openai_client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": prompt}])
        content = res.choices[0].message.content.strip()
        
        # Sicherer Split: Falls OpenAI das Format verhaut, füllen wir auf (Fix für 'list index out of range')
...
def inspiration():
    try:
        ...
        # Versuch, passendes Buch aus Supabase zu finden
        try:
        ...
        # Rest mit KI-Vorschlägen auffüllen
